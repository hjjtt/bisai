...
- **LLM-as-a-Judge 评分** — 多轮采样取中位数 + CoT 推理 + few-shot 校准 + 冗长偏差修正 + 交叉模型一致性校验，交叉模型偏差超阈值自动标记需人工复核
...
> **核查与评分延后**由教师通过批量操作或单条触发统一执行。这样既保护 AI 配额（避免学生反复改稿耗尽），
> 也保证评分公平（截止后统一标准，避免早期提交者与临近截止提交者评分尺度漂移）。

1. **教师创建任务** — 设定任务要求、评价模板和截止时间
2. **学生上传成果** — 上传即触发门禁校验（身份/相关性/内容判定），通过后自动解析文档结构（文本提取、图片 OCR、章节大纲）
3. **AI 智能核查** — 教师批量触发，基于任务要求核查文档规范性与完整性，命中红线问题（FAIL+HIGH）自动熔断跳过评分
4. **AI 智能评分** — 紧随核查自动执行，结合知识库 RAG 检索，按 LLM-as-a-Judge 多轮采样生成评分与评语
5. **教师复核** — 查看 AI 评分（含交叉模型偏差告警、多轮采样详情、结构化覆盖度），可校准调整各指标分数
6. **成绩发布** — 单条或批量发布，支持导出 PDF/Word 报告；学生仅能看到已发布成绩

## 角色与权限

| 角色 | 能力 |
|------|------|
| **学生** | 查看任务 · 上传成果（门禁+解析即时反馈）· 查看提交记录 · 查看已发布成绩与改进建议 · 消息中心 |
| **教师** | 任务管理 · 提交管理 · 文件预览 · 触发核查/评分 · 评分复核 · 评价模板 · 知识库 · 报表中心 · 批量操作（解析/核查/评分/发布） |
...
**数据可见性**：学生仅可见已发布（PUBLISHED）成绩，核查证据（evidence）不对学生暴露；教师异步任务字段对学生过滤敏感信息（errorMessage 等）
...
- 轮询间隔：5 秒 · 最大重试：3 次（递增延迟）· 僵尸清理：普通任务 10 分钟 / Agent 任务 30 分钟
- **批量任务锁**：基于 ConcurrentHashMap + 时间戳 TTL（30 分钟自愈），防止事件丢失导致锁永久泄漏
- **任务去重**：`createTaskIfAbsent` 对同 bizId+type 去重，FAILED 任务重置复用而非新建，避免任务堆积
...
| 后端开发 | ✅ 完成 |
| 前端开发 | ✅ 完成 |
| AI 集成 | ✅ 完成（LLM-as-a-Judge + 动态模型切换） |
...
- [[AGENTS]] — 项目规则、坑点与验证命令（**改动前必读**）
- [[CONTEXT]] — 领域语言与角色定义
- [[软件功能需求分析文档|软件功能需求分析文档]] — 功能需求全集
- [[docs/软件功能设计文档|软件功能设计文档]] — 架构与模块设计
- [[docs/软件产品说明书|软件产品说明书]] — 产品功能说明
- [[docs/部署文档|部署文档]] — 安装与部署
- [[index]] — 知识库索引
